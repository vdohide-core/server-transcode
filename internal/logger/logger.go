...
func (rw *RotatingWriter) rotate() error {
	...
	// Rename with timestamp: server-transcode_20060102_150405.log
...
// it is renamed (e.g. server-transcode_20060102_150405.log) before a
// fresh file is created for the new run.
//
// Runtime rotation: file is rotated automatically when it exceeds 25 MB.
//
// logPath defaults to "logs/server-transcode.log" if empty.
func Init(logPath string) (io.Closer, error) {
	if logPath == "" {
		logPath = "logs/server-transcode.log"
	}
